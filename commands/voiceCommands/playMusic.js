...
module.exports = {
  ...
  callback: async (message, arguments, argsText) => {
    ...
    const musicDuration = searchResult.timestamp
    ...
    if(member.voice.channel) {        
        if(!serverQueue) {
          ...
          musicList.songs.push({musicTitle, musicURL, musicDuration})
          ...
          try {
            ...
            const embeds = new MessageEmbed()
              .setColor("ORANGE")
              .setDescription(`Playing [${musicTitle}](${musicURL}) By <@${author.id}>`)
          ...
          serverQueue.songs.push({musicTitle, musicURL, musicDuration})
          const embeds = new MessageEmbed()
              .setColor("ORANGE")
              .setDescription(`Queued [${musicTitle}](${musicURL}) By <@${author.id}>`)
...
const musicPlayer = async (channel, guild, song) =>{ 
  ...
  musicQueue.connection.play(ytdl(song.musicURL, {filter: 'audioonly'}))
    .on('finish', () => {
