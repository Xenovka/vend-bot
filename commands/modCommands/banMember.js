module.exports = {
  ...
  callback: async (message, arguments, argsText) => {
    ...
    const banDay = +arguments[1]
    const banReason = arguments.slice(2, arguments.length)
    
    if(!mentionedUser) {
      channel.send('You should mention the user you want to ban from the server.')
      return;
    }
    ...
    if(!banReason) {
      ...
      channel.send(`<@${userId}> has been banned for ${banDay} day(s) from the server.`)
      return
    }

    await guild.member(userId).ban({days: banDay, reason: banReason})
    channel.send(`<@${userId}> has been banned for ${banDay} day(s) from the server because ${banReason}.`)
  }
}