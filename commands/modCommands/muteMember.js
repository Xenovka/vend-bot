module.exports = {
  ...
  callback: ({ message, args }) => {
    ...
    const mutedDuration = +args[1];
    const mutedReason = args.slice(2, args.length).join(" ");
    ...
    if (!mutedReason) {
      channel.send(
        `<@${mentionedUser.id}> can't send messages for ${mutedDuration} minutes for no reason.`
      );
    } else {
      channel.send(
        `<@${mentionedUser.id}> can't send messages for ${mutedDuration} minutes because of ${mutedReason}.`
      );
    }
    ...
    setTimeout(() => {
      ...
      channel.send(
        `<@${mentionedUser.id}> already unmuted after getting muted for ${mutedDuration} minutes.`
      );
    }, mutedDuration * 60000);
  }
};
