module.exports = {
  ...
  callback: async (message, arguments, argsText) => {
    ...
    const kickReason = arguments.slice(1, arguments.length).join(' ')

    if (!mentionedUser) {
      channel.send('You should mention the user you want to kick from the guild.')
      return;
    }

    if(!kickReason) {
      await guild.member(mentionedUser.id).kick()
      channel.send(`<@${mentionedUser.id}> has been kicked for the server for no reason.`)
      return
    }
    
    await guild.member(mentionedUser.id).kick(kickReason)
    channel.send(`<@${mentionedUser.id}> has been kicked for the server because ${kickReason}.`)
